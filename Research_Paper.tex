...
\midrule
Total Return & 1.50\% $\pm$ 3.26\% & -0.31\% $\pm$ 1.78\% \\
Sharpe Ratio & 0.1189 $\pm$ 0.1594 & 0.0270 $\pm$ 0.0980 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Alpha Generation vs Absolute Returns}
As depicted in our results, the Baseline PPO experienced higher absolute returns (1.50\%) but with dramatic variance and instability ($\pm$ 3.26\%). In contrast, the Causal PPO maintained a tighter bound on returns (-0.31\% $\pm$ 1.78\%). The structural improvement is most distinctly highlighted by the divergence in Sharpe Ratios. While the absolute Sharpe dropped slightly, the causal agent achieved a much tighter standard deviation tightly clustered around zero, indicating that the returns were achieved with measured variance that is decoupled from simple market momentum swings.
